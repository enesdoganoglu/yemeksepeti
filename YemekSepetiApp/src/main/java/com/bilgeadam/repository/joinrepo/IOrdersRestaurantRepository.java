...
@Repository
public interface IOrdersRestaurantRepository extends JpaRepository<OrdersRestaurant, Long> {
    ...
    @Query(nativeQuery = true, value = "select opi.order_order_id , r.restaurant_name, p.product_name, c.customer_name ,c.customer_surname from customer as c  \n" +
            ...
            "inner join product as p on p.product_id = opi.product_id where r.restaurant_id = ?1"
    )
    List<OrdersRestaurant> restaurantOrders(Long restaurantId);
}
